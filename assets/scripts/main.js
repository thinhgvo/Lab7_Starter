...
function initializeServiceWorker() {
  ...
  if("serviceWorker" in navigator) {
    self.addEventListener('load', () => {                                 // B2
      try {
        const registration = navigator.serviceWorker.register("./sw.js"); // B3
        if(registration.active) {
          console.log("Service Worker has been successfully registered"); // B4
        }
      } catch (error) {
        console.error(error);                                             // B5
      }
    });
  }
...
async function getRecipes() {
  ...
  const newPromise = new Promise((resolve, reject) => {
    RECIPE_URLS.forEach(async element => {   // A4
      try {                                  // A5
        let recipe = await fetch(element);   // A6
        recipes.push(await recipe.json());   // A7 & A8
